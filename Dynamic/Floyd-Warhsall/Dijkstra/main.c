...
{
    int temp;
    //storing a initial value which is not visited
    for(int i=0;i<g->v;i++)
    { 
        if(check[i]==0&&dis[i]!=inf)
        {
            temp=i;
            break;
        }
...
void dijk(int u)
{   dis[u]=0;
    for(int k=0;k<g->v;k++)//comparing the distances
    {
        int temp=compare();
        check[temp]=1;
    for(int i=0;i<g->v;i++)
    {
        if(g->adj[temp][i]!=0 || g->adj[temp][i]!=inf &&check[i]==1)
        {   check[i]==1;
            min_d(temp,i);
        }
    }
